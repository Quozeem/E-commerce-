...
<body>
	<div class="spr-content" style="margin-left: 40px;margin-right:40px">
                                                    ...
                                                    <form method="post" action="{{route('writerview')}}" class="new-review-formx">
													@csrf	<h3 class="spr-form-title">Write a review</h3>
			...
			<input class="spr-form-input spr-form-input-text" type="text" name="author"
             value="{{(Auth::guard('customer')->user()->fname)}}" placeholder="Enter your name" required="">
																</div>
        ...
        value="{{(Auth::guard('customer')->user()->email)}}" placeholder="smith@example.com" required="">
      @foreach((session('productdetails')) as $id)
      @php $reviewid=$id->id ;@endphp
      @endforeach
      <input class="spr-form-input spr-form-input-email" type="hidden" name="procuctid" 
        value="{{ $reviewid}}" placeholder="smith@example.com">
																</div>
															</fieldset>
